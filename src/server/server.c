...
int main()
{
	...
	if ((server = InitServer()) == NULL) {
		write(stderr, "Error of create server", strlen("Error of create server"));
		exit(1);
	}
	...
	switch(msg.flg) {
		...
		case FLG_STEP:
			...
			if (from == 1) {
				...
				shot_rez = CheckData(&msg, &serv->client_2_field , &client_ships);
				...
				shot_rez = CheckData(&msg, &serv->client_1_field , &client_ships);                                
				
				/* ообщение второму игроку */
				if (shot_rez == CELL_MISS)
					server_msg.flg = FLG_WAIT;
				else {
					server_msg.flg = FLG_STEP;
...
void GetIP(SERVER_S *serv)
{
		...
		/*
				Если лень вводить
		*/       
		inet_aton("127.0.0.1", &serv->addr.sin_addr);
		return;
