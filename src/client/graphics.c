 ...
 */
static void GraphCellPrint(WINDOW *cell, int val)
{
	switch(val){
		case CELL_MISS:
 ...
 */
void GraphFieldRefresh(int nfield, struct play_field *pfield)
{
	...
	for (i = 0; i < FIELD_LINES; i++){
		for(j = 0; j < FIELD_COLS; j++){
			...
			if (nfield == FIELD_MY)
				GraphCellPrint(current, pfield->prv[i][j]);
			
			if (nfield == FIELD_ENEMY)
				GraphCellPrint(current, pfield->pub[i][j]);
 ...
 */
void GraphCellRefresh(int nfield, int line, int col, int val)
{
 ...
 */
void GraphCellRepaint(int line, int col, int numpair)
{
 ...
 */
int GraphCellGet(int *line, int *col, struct timeval *time)
{
	...
	GraphCellRepaint(*line, *col, COLOR_CELL_ACTIVE);
	while (1) {   
		...
		GraphCellRepaint(prev_line, prev_col, COLOR_CELL_DEFAULT);
		GraphCellRepaint(*line, *col, COLOR_CELL_ACTIVE);
	}
 ...
 */
void GraphItemPrint(char *title, char **list, int lsize, int selected)
{
	int offset = 0;
	int i, line;
	if (selected > (LINES - 4))
	...
	for (i = offset, line = 3; i < lsize && line < LINES; i++){
		if (i == selected)
 ...
 */
int GraphItemGet(char *title, char **list, int lsize)
{
	int key;            /* Нажатая кнопка   */
	int selected = 0;   /* Выбраный элемент */

	clear();
	GraphItemPrint(title, list, lsize, selected);
	while ((key = getch()) != '\n')
	{
		/**
